...
当下前端项目开发都会基于 webpack 或者 vite 等工具搭建一套工程化环境，在这个过程中我们可以安装相应的 loader 来识别和处理 ts 文件，最后通过相应的命令执行项目的打包，此时得到的就是可直接运行的 js 文件。这个操作，我们放在后面进行描述，语法熟悉阶段，我们使用 ts-node 来完成编译。